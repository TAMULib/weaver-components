import { Component, Input, ViewEncapsulation, Injector, AfterViewInit } from '@angular/core';
import { SafeUrl } from '@angular/platform-browser';
import { WvrBaseComponent } from '../shared/wvr-base-component';

@Component({
  selector: 'wvr-header-element',
  templateUrl: './wvr-header.component.html',
  styleUrls: ['./wvr-header.component.scss'],
  encapsulation: ViewEncapsulation.ShadowDom
})
export class WvrHeaderComponent extends WvrBaseComponent {

  private readonly BROW_BG_CSS_VAR: string = '--brow-nav-bg';
  private readonly BROW_HEIGHT_CSS_VAR: string = '--brow-nav-height';
  private readonly BROW_PADDING_CSS_VAR: string = '--brow-nav-padding';

  private readonly TITLE_BG_CSS_VAR: string = '--title-row-bg';
  private readonly TITLE_HEIGHT_CSS_VAR: string = '--title-row-height';
  private readonly TITLE_PADDING_CSS_VAR: string = '--title-row-padding';

  private readonly LOWER_BG_CSS_VAR: string = '--lower-nav-bg';
  private readonly LOWER_HEIGHT_CSS_VAR: string = '--lower-nav-height';
  private readonly LOWER_PADDING_CSS_VAR: string = '--lower-nav-padding';

  @Input() logoText = 'Weaver Components';
  @Input() headerTitle = 'Weaver Header Component';
  @Input() logoSrc: SafeUrl = this.domSanitizer.bypassSecurityTrustUrl('data:image/svg+xml;base64,');
  @Input() logoHref = "#";

  @Input()
  set browBg(v: string) {
    this.themeSettings.set(this.BROW_BG_CSS_VAR, v);
  }

  get browBg(): string {
    return this.themeSettings.get(this.BROW_BG_CSS_VAR);
  }

  @Input()
  set browHeight(v: string) {
    this.themeSettings.set(this.BROW_HEIGHT_CSS_VAR, v);
  }

  get browHeight(): string {
    return this.themeSettings.get(this.BROW_HEIGHT_CSS_VAR);
  }

  @Input()
  set browPadding(v: string) {
    this.themeSettings.set(this.BROW_PADDING_CSS_VAR, v);
  }

  get browPadding(): string {
    return this.themeSettings.get(this.BROW_PADDING_CSS_VAR);
  }

  @Input()
  set titleBg(v: string) {
    this.themeSettings.set(this.TITLE_BG_CSS_VAR, v);
  }

  get titleBg(): string {
    return this.themeSettings.get(this.TITLE_BG_CSS_VAR);
  }

  @Input()
  set titleHeight(v: string) {
    this.themeSettings.set(this.TITLE_HEIGHT_CSS_VAR, v);
  }

  get titleHeight(): string {
    return this.themeSettings.get(this.TITLE_HEIGHT_CSS_VAR);
  }

  @Input()
  set titlePadding(v: string) {
    this.themeSettings.set(this.TITLE_PADDING_CSS_VAR, v);
  }

  get titlePadding(): string {
    return this.themeSettings.get(this.TITLE_PADDING_CSS_VAR);
  }

  @Input()
  set lowerBg(v: string) {
    this.themeSettings.set(this.LOWER_BG_CSS_VAR, v);
  }

  get lowerBg(): string {
    return this.themeSettings.get(this.LOWER_BG_CSS_VAR);
  }

  @Input()
  set lowerHeight(v: string) {
    this.themeSettings.set(this.LOWER_HEIGHT_CSS_VAR, v);
  }

  get lowerHeight(): string {
    return this.themeSettings.get(this.LOWER_HEIGHT_CSS_VAR);
  }

  @Input()
  set lowerPadding(v: string) {
    this.themeSettings.set(this.LOWER_PADDING_CSS_VAR, v);
  }

  get lowerPadding(): string {
    return this.themeSettings.get(this.LOWER_PADDING_CSS_VAR);
  }

  private themeSettings: Map<string, string> = new Map<string, string>();

  constructor(injector: Injector) {
    super(injector);
  }

  ngOnInit() {
    this.applyTheme();
  }

  private applyTheme(): void {
    this.themeSettings.forEach((varVal, varName) => {
      this.elem.nativeElement.style.setProperty(varName, varVal);
    });
  }

}
