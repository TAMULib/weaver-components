import { Component, Injector, Input, ViewEncapsulation, ElementRef, AfterContentInit } from '@angular/core';
import { DomSanitizer, SafeUrl, SafeHtml } from '@angular/platform-browser';
import { createCustomElement } from '@angular/elements';

@Component({
  selector: 'wvr-header-element',
  templateUrl: './wvr-header.component.html',
  styleUrls: ['./wvr-header.component.scss'],
  encapsulation: ViewEncapsulation.ShadowDom
})
export class WvrHeaderComponent implements AfterContentInit {
  
  title = 'header-component';
  @Input() logoText = ' | Weaver Components';
  @Input() headerTitle = 'Weaver Header Component';
  @Input() logoSrc:SafeUrl = '';

  browNavLinks: HTMLElement[] = new Array<HTMLElement>();
  bottomNavLinks: HTMLElement[] = new Array<HTMLElement>();

  private slotValidation = {
    'browNavLinks': "WVR-LINK",
    'bottomNavLinks': "WVR-LINK"
  }

  constructor(private domSanitizer: DomSanitizer, injector: Injector) {
    this.logoSrc = domSanitizer.bypassSecurityTrustUrl('data:image/svg+xml;base64,');
  }

  safe(html: string): SafeHtml {
    return this.domSanitizer.bypassSecurityTrustHtml(html);
  }

  slotChange($event) {
    const assigned:[] = $event.target.assignedNodes();
    if(assigned.length != this[$event.target.name].length) {
      this[$event.target.name].length = 0;
      assigned.forEach(elem=>{
        if((<HTMLElement>elem).tagName === this.slotValidation[$event.target.name])
        this[$event.target.name].push(elem);
      });
    }
  }

}
